...
import { createTask, deleteTask, fetchBoards, fetchTasks, inviteTeamMember, updateTask } from '../services/api';
...
export default function ProjectOverview({ theme = 'light', onNavigate }) {
  ...
  return <div className={`project-board ${theme === 'dark' ? 'project-board--dark' : ''}`}>
    ...
    <main className="project-board__content">
      <header className="project-board__header"><div>{boards.length > 1 ? <select className="project-board__board-picker" value={boardId} onChange={(event) => setBoardId(event.target.value)}>{boards.map((board) => <option key={idOf(board)} value={idOf(board)}>{board.title}</option>)}</select> : <h1>{selectedBoard?.title || 'Project Name'}</h1>}{selectedBoard?.description && <p>{selectedBoard.description}</p>}</div><button className="project-board__invite" onClick={() => setInviteOpen(true)} disabled={!boardId}>+ Invite</button></header>
