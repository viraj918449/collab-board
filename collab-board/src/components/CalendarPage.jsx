...
export default function CalendarPage({ onLogout, onNavigate, theme = 'light', selectedDate, scheduleItems = [] }) {
  ...
  for (let day = 1; day <= daysInCurrentMonth; day++) {
    ...
    const isSelected = selectedCalendarDate && !Number.isNaN(selectedCalendarDate.getTime())
      && isSameDay(cellDate, selectedCalendarDate);

    calendarCells.push(
      <div 
        key={`curr-${day}`} 
        onClick={() => handleDateClick(day, currentMonthName, currentYear)} 
        style={{ padding: '4px', color: textColor, cursor: 'pointer', borderRadius: '6px', outline: isSelected ? '2px solid #2563eb' : 'none', outlineOffset: '-2px' }}
      >
        {isToday ? (
          <>
