...
export default function Sidebar({ currentView, onNavigate, onLogout, theme = 'light' }) {
  return <aside className={`sidebar sidebar--${theme}`} aria-label="Main navigation">
    ...
    <nav className="sidebar__nav">{navigation.map(([view, icon, label]) => <button key={view} type="button" className={`sidebar__link ${currentView === view ? 'sidebar__link--active' : ''}`} onClick={() => onNavigate(view)}><span aria-hidden="true">{icon}</span><span>{view === 'profile' ? 'Update Profile' : label}</span></button>)}</nav>
